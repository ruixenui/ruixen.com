...
export const generatePureCss = (state: GlassState) => {
  ...
  const hoverShadow = state.hoverShadow ? shadowFor(state.shadowIntensity * 1.1) : boxShadow
...
${state.hoverEnabled ? `.glass-card:hover {
  background: ${hoverBg};
...
export const generateTailwindSnippet = (state: GlassState) => {
  ...
  const hoverBg = rgba(withAlpha(state.bgColor, state.hoverOpacity))
  const hoverShadow = state.hoverShadow ? shadowFor(state.shadowIntensity * 1.1) : shadow
  ...
  const base = [
    ...
    `border-[${border}]`,
    `border-[${borderWidth}]`,
    shadow !== "none" ? `shadow-[${shadow.replaceAll(" ", "_")}]` : `shadow-none`,
  ]
...
export const decodeStateFromQuery = (queryValue: string): GlassState | null => {
  try {
    const b64 = queryValue.startsWith(ENCODING_PREFIX) ? queryValue.slice(ENCODING_PREFIX.length) : queryValue
