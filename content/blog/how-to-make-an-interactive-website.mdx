...
image: https://pub-940ccf6255b54fa799a9b01050e6c227.r2.dev/gradients/shade_shifters/shade-shifters-14.png
...
/* app/globals.css */
:root {
  --bg: #ffffff; --fg: #0b0b0b; --brand: oklch(56% 0.2 262);
  --radius: 12px;
}
:root[data-theme="dark"] {
  --bg: #0b0b0b; --fg: #f5f5f5; --brand: oklch(70% 0.18 262);
}
  ...
  return (
    <header className="sticky top-0 z-40 border-b bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50 dark:bg-black/30">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        <Link href="/" className="font-semibold">Ruixen</Link>
        <nav className="flex items-center gap-2">
          <Link href="/work" className="px-3 py-2">Work</Link>
          <Link href="/blog" className="px-3 py-2">Blog</Link>
...
export default function Demo() {
  return (
    <div className="mx-auto max-w-4xl space-y-10 p-6">
      ...
      <Dialog>
        <DialogTrigger className="rounded-md border px-3 py-2">Open modal</DialogTrigger>
...
export function ScrollProgress() {
  ...
  return <div className="fixed inset-x-0 top-0 h-1 bg-brand" style={{ width: w + "%" }} />;
}
```
...
export function SignupForm() {
  ...
  return (
    <form onSubmit={submit} className="space-y-2">
      ...
      />
      {error && <p id="email-err" className="text-sm text-red-600">{error}</p>}
      <button className="rounded-md bg-brand px-4 py-2 text-white">Sign up</button>
    </form>
  );
}
```
