...
export const DEFAULT_COMPONENTS_ruixenui = "@/components/ruixen";
...
export async function resolveConfigPaths(cwd: string, config: RawConfig) {
  ...
  const ruixen = `${components}/ruixen`;

  const newAliases = {
    ui: `${config.aliases.components}/ui`,
    ruixen: `${config.aliases.components}/ruixen`,
  };
  ...
  const newConfig = {
    ...
    resolvedPaths: {
      ...
      ruixen,
      //   ? await resolveImport(config.aliases["ruixen"], tsConfig)
