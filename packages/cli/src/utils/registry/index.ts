...
const baseUrl = process.env.COMPONENTS_REGISTRY_URL ?? "https://ruixen.com";
const proBaseUrl = process.env.PRO_REGISTRY_URL ?? "https://ruixen.com";
...
export async function getRegistryIndexruixenui(env?: boolean) {
  try {
    ...
    console.error(error);
    throw new Error("Failed to fetch components from Ruixen UI registry.");
  }
}
...
export async function resolveTreeWithShadcn(
...
): Promise<{ shadcnTree: theTree; ruixenuiTree: theTree }> {
  const shadcnTree: theTree = [];
  const ruixenuiTree: theTree = [];
  ...
  for (const name of names) {
    if (!calledByShadcn) {
      ...
      entry && ruixenuiTree.push(entry);

      if (entry && entry.registryDependencies) {
        const {
          ruixenuiTree: ruixenuiTreeDependencies,
        ...
        shadcnTree.push(...shadcnTreeDependencies);
        ruixenuiTree.push(...ruixenuiTreeDependencies);
      }
    } else {
  ...
  return {
    ...
    ruixenuiTree: ruixenuiTree.filter(
      (component, index, self) =>
        self.findIndex((c) => c.name === component.name) === index,
    ),
  };
...
export async function fetchTree(tree: theTree, env?: string) {
  try {
    ...
    // {baseUrl}/registry/components/ruixen/[name].json.
    ...
    throw new Error(`Failed to fetch tree from Ruixen UI registry.`);
  }
}
