...
export const init = new Command()
  .name("init")
  .description("Initialize your project with shadcn-ui config & ruixen-ui")
  ...
  .action(async (opts) => {
    try {
      ...
      logger.info(
        `${chalk.green(
          "Success!",
        )} Project initialization completed. You may now add both shadcn-ui and ruixen-ui components.`,
      );
      logger.info("");
    } catch (error) {
      handleError(error);
...
export async function promptForConfig(
  ...
  const config = rawConfigSchema.parse({
    ...
    aliases: {
      ...
      ruixen: `${options.components}/ruixen`,
    },
  });
