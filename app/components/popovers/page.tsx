...
import { popoverComponents } from './PopoverComponentsExport';
  ...
  return (
    <div className="dark:bg-black bg-white w-full min-h-screen flex flex-col items-start p-2 pt-24">
      ...
      <div className="w-full mt-8 grid grid-cols-1 sm:grid-cols-4 gap-4 relative inner-container">
        {popoverComponents.map((item, index) => (
      ...
      {openIndex !== null && (
               <div ref={showcaseRef} className="w-full mt-12">
                 <TemplateShowcasePage
                   title={popoverComponents[openIndex].name}
                   description={popoverComponents[openIndex].description}
                   preview={popoverComponents[openIndex].preview}
                   code={popoverComponents[openIndex].code}
                 />
               </div>
             )} 
    </div>
