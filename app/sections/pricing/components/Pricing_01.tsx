...
export const codeStringPricing_01 = `"use client";

import { CheckIcon } from "lucide-react";
import { useState } from "react";
import { motion } from "framer-motion";

export default function Pricing_Ruixen() {
  ...
  return (
    <section className=\\"py-20 bg-background text-foreground px-4\\">
      <div className=\\"max-w-5xl mx-auto text-center mb-10\\">
        <h2 className=\\"text-3xl sm:text-4xl font-bold text-black dark:text-white\\">Pricing Plans for Every Stage</h2>
        <p className=\\"mt-4 text-muted-foreground max-w-2xl mx-auto text-base\\">
          Whether you're launching or scaling, Ruixen UI supports your journey with expertly crafted UI & services.
        </p>
      ...
      <div className=\\"grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto\\">
        {plans.map((plan, i) => (
          <motion.div
            ...
            className={\`rounded-xl border border-muted bg-white dark:bg-zinc-900 text-black dark:text-white p-4 sm:p-6 flex flex-col \${plan.popular ? \\"ring-2 ring-primary shadow-md\\" : \\"\\"
              }\`}
          >
            <div className=\\"mb-4\\">
              <span className={\`text-xs uppercase font-semibold px-2 py-1 rounded \${plan.labelColor}\`}>
                {plan.label}
              </span>
            </div>
