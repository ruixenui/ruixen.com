...
export function CodeBlockCommand({
  ...
  return (
    <div className="relative w-full mt-6 max-h-[650px] overflow-hidden rounded-xl border border-zinc-200 bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-900">
      <Tabs
        className="w-full"
            ...
            {Object.entries(tabs).map(([key, value]) => {
              return (
                <TabsTrigger
                  key={key}
                  value={key}
                  className="rounded-none border-b border-transparent bg-transparent p-0 pb-1.5 font-mono text-zinc-400 data-[state=active]:border-b-zinc-900 data-[state=active]:bg-transparent data-[state=active]:text-zinc-900 dark:text-zinc-400 dark:data-[state=active]:border-b-zinc-50 dark:data-[state=active]:text-zinc-50"
                >
                  {key}
                </TabsTrigger>
              );
        ...
        {Object.entries(tabs).map(([key, value]) => {
          return (
            <TabsContent key={key} value={key} className="mt-0">
              <pre className="px-4 py-5 overflow-x-auto" style={{ filter: "none" }}>
                <code
                  className="relative font-mono text-sm leading-none text-zinc-800 dark:text-zinc-100"
                  data-language="bash"
                >
                  {value}
                </code>
      ...
      <Button
        size="icon"
        variant="ghost"
        className="absolute right-2.5 top-2 z-10 h-6 w-6 text-zinc-500 hover:bg-zinc-200 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-700 dark:hover:text-zinc-50 [&_svg]:h-3 [&_svg]:w-3"
        onClick={copyCommand}
      >
