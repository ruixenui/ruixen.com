          ...
          <div className="px-6 pb-6 space-y-5">
            ...
            <section>
              ...
              <Tabs
                ...
                onValueChange={(nv) =>
                  onChange({
                    background: { ...value.background, type: nv as any },
                  })
                }
              >
                ...
                <TabsContent value="image" className="space-y-3 mt-3">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="img"
                      placeholder="https://..."
                      value={value.background.imageUrl ?? ""}
                      onChange={(e) =>
                        onChange({
                          background: {
                            ...value.background,
                ...
                <TabsContent value="grid" className="space-y-3 mt-3">
                  <SliderControl
                    ...
                    max={64}
                    step={1}
                    value={[value.background.gridSize ?? 24]}
                    onValueChange={(v) =>
                      onChange({
                        background: { ...value.background, gridSize: v[0] },
                      })
                    }
                  />
                </TabsContent>
                <TabsContent value="noise" className="space-y-3 mt-3">
                  <SliderControl
                    ...
                    step={1}
                    value={[
                      Math.round((value.background.noiseOpacity ?? 0.08) * 100),
                    ]}
                    onValueChange={(v) =>
                      onChange({
                        background: {
                          ...value.background,
                          noiseOpacity: v[0] / 100,
                        },
                      })
                    }
                    suffix="%"
              ...
              <div className="mt-4">
                <ToggleControl
                  label="Dark mode canvas"
                  checked={!!value.background.darkMode}
                  onCheckedChange={(c) =>
                    onChange({
                      background: { ...value.background, darkMode: !!c },
                    })
                  }
                />
              </div>
            ...
            <section className="space-y-3">
              ...
              <SliderControl
                ...
                onValueChange={(v) => onChange({ borderWidth: v[0] })}
                suffix="px"
              />
              <ColorPicker
                label="Border Color"
            ...
            <section className="space-y-3">
              ...
              <ToggleControl
                ...
                onCheckedChange={(c) => onChange({ shadow: !!c })}
              />
              <SliderControl
                ...
                max={100}
                step={1}
                value={[Math.round(value.shadowIntensity * 100)]}
                onValueChange={(v) => onChange({ shadowIntensity: v[0] / 100 })}
                suffix="%"
              />
            </section>
            ...
            <section className="space-y-3">
              ...
              <ToggleControl
                ...
                checked={value.gradientOverlay}
                onCheckedChange={(c) => onChange({ gradientOverlay: !!c })}
              />
              <SliderControl
                disabled={!value.gradientOverlay}
                label="Gradient Angle"
                min={0}
                max={360}
                step={1}
                value={[value.gradientAngle]}
                onValueChange={(v) => onChange({ gradientAngle: v[0] })}
                suffix="°"
              />
            </section>
            ...
            <section className="space-y-3">
              ...
              <ToggleControl
                ...
                checked={value.hoverEnabled}
                onCheckedChange={(c) => onChange({ hoverEnabled: !!c })}
              />
              <SliderControl
                disabled={!value.hoverEnabled}
                label="Hover Blur"
                min={0}
                max={80}
                step={1}
                value={[value.hoverBlur]}
                onValueChange={(v) => onChange({ hoverBlur: v[0] })}
                suffix="px"
              />
              <SliderControl
                disabled={!value.hoverEnabled}
                ...
                max={100}
                step={1}
                value={[Math.round(value.hoverOpacity * 100)]}
                onValueChange={onHoverOpacity}
                suffix="%"
              />
              <ToggleControl
                disabled={!value.hoverEnabled}
                label="Hover Shadow"
                checked={value.hoverShadow}
                onCheckedChange={(c) => onChange({ hoverShadow: !!c })}
              />
            </section>
          </div>
        </ScrollArea>
