...
export default function PreviewPanel({ value }: Props) {
  ...
  const [theme, setTheme] = useState<"light" | "dark">(value.background.darkMode ? "dark" : "light")
  ...
  const canvasBgStyle = useMemo(() => {
    ...
    if (b.type === "image") {
      return { backgroundImage: `url(${b.imageUrl})`, backgroundSize: "cover", backgroundPosition: "center" } as const
    }
    if (b.type === "grid") {
      const size = b.gridSize ?? 24
      const color = theme === "dark" ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.06)"
      return {
    ...
    if (b.type === "noise") {
      ...
      return { backgroundImage: `url("${noise}")`, backgroundSize: "auto" } as const
    }
    // frosted: subtle gradient blobs
    const blob1 = theme === "dark" ? "rgba(96, 165, 250, .22)" : "rgba(147, 197, 253, .35)"
    const blob2 = theme === "dark" ? "rgba(244, 114, 182, .16)" : "rgba(251, 113, 133, .24)"
    return {
      backgroundImage:
        `radial-gradient(800px 400px at 10% 10%, ${blob1}, transparent 60%), radial-gradient(800px 400px at 90% 90%, ${blob2}, transparent 55%)`,
    } as const
  ...
  const cardStyle = useMemo(() => {
    ...
    const normal: React.CSSProperties = {
      ...
      boxShadow: value.shadow ? `0 10px 30px rgba(0,0,0,${0.06 + value.shadowIntensity * 0.24})` : "none",
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            <Button variant={simulateHover ? "default" : "outline"} size="sm" onClick={()=> setSimulateHover((s)=>!s)}>
              {simulateHover ? "Hover" : "Normal"}
            </Button>
            <Button variant={mobile ? "default" : "outline"} size="sm" onClick={()=> setMobile((s)=>!s)}>
      ...
      <CardContent className="p-4">
        <div className={canvasClass} style={{ ...canvasBgStyle, height: mobile ? 280 : 320 }}>
          {/* Example content behind the glass card */}
          <div className="pointer-events-none absolute -left-10 -top-10 h-48 w-48 rounded-full blur-3xl opacity-40"
               style={{ background: "linear-gradient(45deg, #60a5fa, #f472b6)" }} />
          <div className="pointer-events-none absolute -right-20 -bottom-16 h-56 w-56 rounded-full blur-3xl opacity-40"
