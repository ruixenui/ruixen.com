...
export default function HistoryManager({ value, onChange }: Props) {
  ...
  // Persist
  useEffect(() => {
    localStorage.setItem(LS_KEY, JSON.stringify({ undo: undoStack, redo: redoStack }))
  }, [undoStack, redoStack])
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center gap-2 flex-wrap">
          <Button size="sm" onClick={undo} disabled={!undoStack.length} variant="outline">
            Undo
          </Button>
          <Button size="sm" onClick={redo} disabled={!redoStack.length} variant="outline">
            Redo
          </Button>
