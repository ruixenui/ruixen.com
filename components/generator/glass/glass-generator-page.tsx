...
import { defaultGlassState, GlassState, encodeStateToQuery, decodeStateFromQuery } from "@/lib/glass"
...
export default function GlassGeneratorPage() {
  ...
  return (
    <main className="min-h-screen w-full max-w-7xl mx-auto">
      ...
      {/* Presets */}
      <section id="presets">
        <PresetGallery builtins={BUILT_IN_PRESETS} onApply={(preset) => setState(preset)} current={state} />
      </section>
...
function SeoFooterArticle() {
  return (
    <article className="prose prose-neutral dark:prose-invert max-w-none prose-headings:font-bold">
      ...
      <p>
        Glassmorphism pairs translucent panels with blurred, colorful backgrounds to create depth without heavy
        gradients or 3D. The style works best in cards, popups, and overlays where you can keep copy short, contrast high, and motion subtle. Our generator outputs production‑ready CSS and Tailwind so you can move from idea to implementation in seconds.
      </p>
      <h3>Principles</h3>
      <ul>
        <li><strong>Contrast first:</strong> Always check text against the blurred background; add an overlay or increase opacity when needed.</li>
        <li><strong>Focus areas:</strong> Reserve glass for primary surfaces that benefit from depth.</li>
        <li><strong>Performance minded:</strong> backdrop filters are GPU‑accelerated but can be heavy if overused.</li>
      </ul>
      <h3>Backdrops & Overlays</h3>
      <p>
        Most glass cards combine a translucent fill, a subtle border, and a soft shadow. The border helps edges read
        crisply; the shadow lifts the card away from the canvas. A gradient overlay can add polish and “specular” highlights that feel tactile without distracting animation.
      </p>
      <h3>Accessibility</h3>
      <p>
        Avoid placing dense content behind glass. If blur is low, keep the background quiet (grid or soft gradient).
        Provide dark and light modes and ensure interactive targets remain large and consistent. Keyboard users should be able to tab through sliders and toggles; this tool relies on shadcn/ui’s accessible primitives for that reason.
      </p>
      <h3>Use Cases</h3>
      <p>
        Dashboards, media overlays, and onboarding surfaces are ideal. For long‑form layouts or data‑heavy tables,
        consider a solid background with subtle elevation instead.
      </p>
      <h3>Production Tips</h3>
      <ul>
        <li>Prefer server or build‑time images for hero backgrounds; lazy‑load decorative assets.</li>
        <li>Use Tailwind’s arbitrary values for one‑off blurs and shadows; safelist if you generate classes dynamically.</li>
        <li>Keep transitions short; hover states should feel crisp, not floaty.</li>
      </ul>
      <p>
