...
export default function PresetGallery({
  ...
  const apply = (
    layers: ReturnType<typeof materialElevation> | ReturnType<typeof neumorph>,
  ) => {
    setState({
      ...state,
      manual: true,
      layers,
      base: { ...state.base, layers: Math.min(5, layers.length) },
    });
  };
                      ...
                      <div
                        ...
                        style={{
                          boxShadow: layers
                            .map(
                              (l) =>
                                `${l.inset ? "inset " : ""}${l.x}px ${l.y}px ${l.blur}px ${l.spread}px rgba(0,0,0,${l.opacity})`,
                            )
                            .join(", "),
                        }}
                      />
          ...
          <TabsContent value="soft" className="mt-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              {[
                ...
                const layers = neumorph(
                  state.preview.background as any,
                  p.inset,
                );
                return (
                  <button
...
function PresetGrid({
  ...
  items: { name: string; layers: ReturnType<typeof materialElevation> }[];
  onPick: (l: any) => void;
}) {
