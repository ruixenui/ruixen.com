...
// ============================================================================
// Extended Tooltip with multiple items (for comparison views)
// ============================================================================

interface ChartTooltipMultiProps {
  active?: boolean;
  payload?: Array<{
    name: string;
    value: number;
...
function ChartTooltipMulti({
  active,
  payload,
  indicator = "dot",
  valueFormatter,
  label,
}: ChartTooltipMultiProps) {
  if (!active || !payload?.length) return null;

  return (
    <div className="rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl min-w-[8rem]">
      {label && (
        <div className="mb-1.5 font-medium text-foreground">{label}</div>
      )}
      <div className="grid gap-1.5">
...
// ============================================================================
// 6. Funnel Chart - Tooltip Indicator Demo (dot)
// ============================================================================

function FunnelTooltipDotChart({
  data,
  className,
  valueFormatter = defaultValueFormatter,
}: FunnelChartBaseProps) {
  const chartData = data.map((item, index) => ({
    ...item,
    fill: item.fill || getColor(index),
  }));
    ...
    <div className={cn("w-full", className)}>
      <ResponsiveContainer width="100%" height="100%">
        <RechartsFunnelChart>
          <Tooltip
            content={
              <ChartTooltip indicator="dot" valueFormatter={valueFormatter} />
            }
            cursor={false}
          />
          <Funnel
...
// ============================================================================
// 10. Funnel Chart - Tooltip Indicator Demo (line)
// ============================================================================

function FunnelTooltipLineChart({
  data,
  className,
  valueFormatter = defaultValueFormatter,
}: FunnelChartBaseProps) {
    ...
    <div className={cn("w-full", className)}>
      <ResponsiveContainer width="100%" height="100%">
        <RechartsFunnelChart>
          <Tooltip
            content={
              <ChartTooltip indicator="line" valueFormatter={valueFormatter} />
            }
            cursor={false}
          />
          <Funnel
...
// ============================================================================
// 11. Funnel Chart - Tooltip Indicator Demo (dashed)
// ============================================================================

function FunnelTooltipDashedChart({
  data,
  className,
  valueFormatter = defaultValueFormatter,
}: FunnelChartBaseProps) {
...
export {
  ...
  FunnelActiveChart,
  FunnelTooltipDotChart,
  FunnelTooltipLineChart,
  FunnelTooltipDashedChart,
  ChartTooltip,
  ChartTooltipMulti,
  ChartLegend,
